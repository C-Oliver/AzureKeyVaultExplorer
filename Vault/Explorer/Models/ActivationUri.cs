...
namespace Microsoft.Vault.Explorer
{
    ...
    public class ActivationUri : VaultLinkUri
    {
        ...
        {
            // ClickOnce (ApplicationDeployment) is not available in .NET 10; use command-line args only
            string? vaultUri = (Environment.GetCommandLineArgs().Length == 2) ? Environment.GetCommandLineArgs()[1] : null;
        ...
        public static void RegisterVaultProtocol()
        {
            try
            {
                // Register vault: protocol for current user
